...
function Module.BanUserId( UserId, BanProperties )
	...
	local Profile = SystemsContainer.DataServer.LoadUserId(UserId)
	if Profile then
		BanProperties = Module.ConcileBanProperties(BanProperties)
		Profile.Data.Banned = BanProperties
		table.insert(Profile.Data.BanHistory, BanProperties)
		Profile:Release()
	end
end
...
function Module.Start()
	...
	if not success then
		warn("Failed to subscribe to topic 'BanServiceNotif' on MessagingService.\n" .. err)
	end
end
